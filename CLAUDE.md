...
**Core components:**
- `openapi.yaml` — OpenAPI 3.0.3 spec defining `GET` and `PATCH` on `/statutory-actions/{statActionId}` with statutory action metadata + parcel bulk operations
...
│   ├── openapi.yaml               # API specification (GET/PATCH /statutory-actions/{statActionId}, discriminator pattern)
...
### 4. MSW Mock Backend
The example uses MSW v2 to intercept fetch requests in the browser at `https://api.statutory-actions.local/v1/statutory-actions/{statActionId}`. This allows testing the full CRUD flow without a real backend.

**Key files:**
- `src/mocks/handlers.ts` — GET `/statutory-actions/{statActionId}` (returns statutory action metadata and parcel array) and PATCH `/statutory-actions/{statActionId}` (processes parcel operations, updates in-memory state)
